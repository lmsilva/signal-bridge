...
test('the wide Settings cards span the grid and column up inside', () => {
  ...
  assert.match(html, /styles\.css\?v=signal312/);
  assert.match(html, /settings-filter\.js\?v=signal307/);
  assert.match(html, /app\.js\?v=signal312/);
...
test('household login, /user/ gate, and permission 403s', async () => {
  ...
  try {
    ...
    assert.match(fs.readFileSync(path.join(realWebRoot, 'user', 'index.html'), 'utf8'), /vestaboard-sim-ui\.js\?v=signal313/);
    assert.match(
