...
test('crypto market push delivers CoinGecko quotes and settings can retune the list', async () => {
  const calls = [];
  const config = makeConfig({
    cryptoMarketFetchImpl: async (url) => {
  ...
  const { webServer, base, sent } = await startTestServer({ config });
  try {
    const settings = await getJson(base, '/api/crypto-market/settings');
    assert.equal(settings.status, 200);
    assert.equal(settings.body.source, 'CoinGecko');
    assert.equal(settings.body.settings.symbols.length, 10);

    const saved = await postJson(base, '/api/crypto-market/settings', {
    ...
    assert.equal(pushed.status, 200, pushed.body?.error);
    assert.equal(pushed.body.type, 'crypto.market');
    assert.equal(pushed.body.quotes.length, 3);
    assert.equal(pushed.body.quotes[0].symbol, 'BTC');
    assert.equal(pushed.body.quotes[0].direction, 'down');
    assert.equal(sent.length, 1);
    assert.equal(sent[0].type, 'crypto.market');
...
test('the wide Settings cards span the grid and column up inside', () => {
  ...
  assert.match(css, /\.vb-house-dwell-field \{/);
  assert.match(css, /\.vb-house-dwell-controls \{/);
  assert.match(css, /\.vb-house-policy-row > \.btn/);
  assert.match(js, /\/api\/vestaboards\/house/);
  assert.match(js, /function vbApplyHouse\(/);
  ...
  assert.match(html, /Now<\/strong> replaces what is showing/);
  assert.match(js, /nowSpan\.textContent = 'Now'/);
  assert.match(js, /immediate: rule\.immediate !== false/);
  ...
  assert.match(js, /\/api\/vestaboard-sim\/queue\/clear/);
  assert.match(js, /\/api\/vestaboards\/release-holds/);
  assert.match(js, /function vbApplyQueue\(/);
  assert.match(js, /cutting in/);
  assert.match(js, /function vbSettleBoard\(/);
  assert.match(js, /still waiting on its stagger delay/);
  assert.match(js, /Hearing the flip without seeing flaps is worse/);
  ...
  assert.match(html, /id="crypto-market-settings-card"/);
  assert.match(html, /id="btn-crypto-market-push"/);
  ...
  assert.match(js, /Does not finish on one screen/);
  ...
  assert.match(js, /\/api\/crypto-market\/settings/);
  assert.match(js, /\/api\/push\/crypto-market/);
  ...
  assert.match(css, /\.crypto-market-settings-card/);
...
test('admin has a Scheduler tab with schedule, activity, simulation and settings views', () => {
  ...
  for (const id of [
    ...
    'sched-display-filter', 'sched-kind-filter', 'sched-group-fold', 'sched-rule-sheet', 'sched-week-grid',
  ...
  assert.match(js, /function schedSuggestedTarget/);
  assert.match(js, /function setAllSchedGroupsCollapsed/);
  assert.match(js, /function visibleSchedGroupNames/);
  assert.match(js, /data-sched-group-fold/);
  assert.match(html, /id="sched-group-fold"/);
  assert.match(html, /data-sched-group-fold="expand"/);
  assert.match(html, /data-sched-group-fold="collapse"/);
...
test('household login, /user/ gate, and permission 403s', async () => {
  ...
  try {
    ...
    assert.match(
      ...
      'simulator flip must snap blank flaps immediately so spaces are not filled by the drum',
    );
    assert.match(
      fs.readFileSync(path.join(realWebRoot, 'vestaboard-sim-ui.js'), 'utf8'),
      /waiting on its stagger delay/,
      'a Skip/state paint must not cancel flaps still waiting to start',
    );
