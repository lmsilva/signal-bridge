...
async function startHarness({ withSimulator = true, withHub = false } = {}) {
  ...
  if (withHub) {
    ...
    // would refuse a shopping/weather submit after bedtime — both on the house
    // queue and on the simulator board the fan-out still consults.
    hub.queueFor('sim')?.setConfig({
      quietHours: { start: '22:00', end: '07:00', enabled: false },
      rateWindowSeconds: 0,
    });
    const simBoard = hub.settings.get('sim');
    if (simBoard) {
      hub.settings.upsert({
        ...simBoard,
        quietHours: { start: '22:00', end: '07:00', enabled: false },
...
test('the simulator page walks the drum slowly and can click', () => {
  ...
  assert.match(js, /function vbHoldsPinned\(/);
...
test('the shared simulator UI flips tiles and can play the house clip', () => {
  const js = fs.readFileSync(path.join(__dirname, '..', 'src', 'web', 'vestaboard-sim-ui.js'), 'utf8');
  const bezel = fs.readFileSync(path.join(__dirname, '..', 'src', 'web', 'vestaboard-bezel.css'), 'utf8');
  const userHtml = fs.readFileSync(path.join(__dirname, '..', 'src', 'web', 'user', 'index.html'), 'utf8');
  const userJs = fs.readFileSync(path.join(__dirname, '..', 'src', 'web', 'user', 'app.js'), 'utf8');
  const userCss = fs.readFileSync(path.join(__dirname, '..', 'src', 'web', 'user', 'styles.css'), 'utf8');
  assert.match(js, /function applyLayout/);
  assert.match(js, /function drumSteps/);
  assert.match(js, /function runFlips/);
  assert.match(js, /MAX_DRUM_STEPS/);
  assert.match(js, /function playCascade/);
  assert.match(js, /function startRateCountdown/);
  ...
  assert.match(js, /function holdsPinned\(/);
