...
**Last updated:** 2026-07-28 (Alarm device location)
...
| `photo.slideshow` | `PhotoSlideshowPanel` — plays through `slideshow.photos[]` once at `slideshow.secondsPerPhoto` (default 5) via the shared-photos page (photo-sampled mat, large print-bordered stage, **elapsed-fill L→R / T→B** progress rail, bottom QR plate “Scan to open”); shell title/backdrop/countdown hidden — status shows `NEXT IN Xs` / `LAST PHOTO`; auto-dismiss still fires after one pass |
