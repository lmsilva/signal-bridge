...
- 2026-07-26: **Scan-QR badge stays on-canvas (portrait + landscape)** — the slideshow / shared-photo "Scan for this photo" badge was anchored to raw `screen_w`/`screen_h` with a centered caption, so in landscape the QR and label clipped off the right/bottom edges. Both now share `BasePanel._photo_stage_geometry` / `_draw_scan_qr_badge`: landscape reserves a right gutter for the QR (photo centers in the remaining width), portrait keeps the photo full-width with the QR tucked into the bottom-right of the *content frame*; the caption is right-aligned (`anchor="se"`) so it grows left into free space instead of spilling off-screen.
