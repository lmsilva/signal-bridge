...
**Last updated:** 2026-07-28 (Now Playing progress)
...
| `air-quality.query` | Owns chrome — SOURCE/Alexa + AIR QUALITY pill; large score ring (arc = score/100) + rating in band colour; proportional GOOD/FAIR/POOR/SEVERE scale (65/35/15 cutoffs, left→right); five metric cells; BY ROOM rows with score bars (landscape: ring left, metrics+rooms right) |
| `timer.snapshot` | Owns chrome — SOURCE/Alexa + TIMERS pill; count ladder (1→760 / 3→560 / 4→420 landscape rings; portrait stacks 2–3); soonest-first with accent on next-to-fire only (others muted); amber/red escalation under 60s/10s; 5+ = hero ring + THEN rows with per-row drain bars; unlabelled timers show `15 MIN` / `1 HR` |
| `alarm.snapshot` | Owns chrome — SOURCE/Alexa + ALARMS pill; next alarm as hero (`6:30` / `AM` / `IN 8H 49M`) + ALSO SET rows with recurrence chips |
| `shopping-list.snapshot` | Owns chrome — SOURCE/Alexa + SHOPPING LIST pill; landscape two-column large items (no cards), portrait single column; pages when needed |
| `music.playing` | Large album art + track info — sized from available space (not a fixed cap) so it fills portrait; two-column art-left/text-right layout in landscape; when `music.mediaLengthSec` / `mediaProgressSec` / `progressAt` are present, shows `M:SS left · M:SS` with a draining rail that ticks every second and auto-dismisses when remaining hits 0 |
...
| `guest.photobooth` | `GuestPhotoboothPanel` — dual-QR welcome: Step 1 Wi‑Fi join + Step 2 guest booth URL from `guestPhotobooth.{wifi,booth}`; portrait stacks the cards with a "then" cue, landscape places them side by side; display seconds bypass `maxDisplaySeconds` so guests have time to scan both |
| `photo.slideshow` | `PhotoSlideshowPanel` — plays through `slideshow.photos[]` once at `slideshow.secondsPerPhoto` (default 5) via the shared-photos page (photo-sampled mat, large print-bordered stage, segmented/draining progress rail, bottom QR plate “Scan to open”); shell title/backdrop/countdown hidden — status shows `NEXT IN Xs` / `LAST PHOTO`; auto-dismiss still fires after one pass |
