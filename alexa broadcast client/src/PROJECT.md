...
| `src/steam_now_playing_panel.py` | Persistent Steam Now Playing overlay (portrait-first); **aspect-aware hero** (portrait library vs landscape header — no letterboxing), 90% wide column, cover-fit art + soft glow + corner brackets, title+credit on one row with tags under, dedicated large screenshot band, footer with vertical dividers; `steam.now-playing` / close |
