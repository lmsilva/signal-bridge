...
**Last updated:** 2026-07-29 (Music dismiss guard + upload sidebar)
...
| `src/steam_now_playing_panel.py` | Persistent Steam Now Playing — fixed 1000×1100 art stage (blur ambient + contain crisp, corner ticks only), STEAM chip in tag row (never on art), static description clamp, screenshots + stats pinned to bottom; `steam.now-playing` / close |
...
| `shopping-list.snapshot` | Owns chrome — SOURCE/Alexa + SHOPPING LIST pill; density ladder keeps large type for ~10 items, steps down (3-col landscape / 2-col portrait) to fit 20–30+ on one screen when possible, then pages; smoke `shopping-list` / `shopping-list-many` |
...
| `tesla-battery.query` | Owns chrome — VEHICLE/model + UPDATED header; Model Y hero, large %/range, ticked bordered gauge with charge-limit marker, charging chip and optional limit/last-charge/range-added rows; Fleet API errors and **stale cache** retain their status banner |
...
| `guest.photobooth` | `GuestPhotoboothPanel` — owns NETWORK/SSID + GUEST SNAPS chrome; dual bordered QR plates (Wi‑Fi join + guest booth URL) stack in portrait and pair side-by-side in landscape; display seconds bypass `maxDisplaySeconds` so guests have time to scan both |
