...
**Last updated:** 2026-08-07 (Steam idle clock ignores stuck gameid)
...
| `src/parser.js` | Detects announce/broadcast utterances; two-step prompt pairing; dedup; uses `broadcast-parse` to prefer a single ASR fragment and strip duplicated `", broadcast …"` echoes |
...
| `steam.now-playing` | Auto: persistent game card from (1) Steam profile **`gameid`**, else (2) fresh local presence `steamAppId`, else (3) OwnedGames activity that **advanced past an idle baseline** and is within `STEAM_INFER_FROM_RECENT_SEC` (default 180) — baseline is seeded on boot and updated on every session end so quit-time `rtime` cannot reopen the overlay. Open sessions close after `STEAM_RECENT_PLAY_STAGNANT_SEC` (default 150) without OwnedGames playtime/rtime growth — bare `gameid` / stuck `RunningAppID` do **not** refresh that idle clock. Announce without `steamAppId` clears that host’s presence. `STEAM_REQUIRE_PRESENCE=0` (default) shows for any PC. Manual Auth preview may use OwnedGames last-played (dismissible). Close via `steam.now-playing.close` |
...
```bash
npm test                    # bridge only (885 tests)
run_all_tests.bat           # repo root — bridge + Windows client (885 + 405)
```
...
Client tests in `alexa broadcast client/test/test_*.py` — 405 tests; see client `PROJECT.md` Testing section. Install via `requirements-test.txt` (pulls `requirements.txt`).
...
- 2026-08-07: **Steam idle clock only trusts OwnedGames growth** — `lastActivityAt` no longer resets from bare profile `gameid` or local presence (both can stick after quit). Open sessions close after `STEAM_RECENT_PLAY_STAGNANT_SEC` without playtime/rtime growth even if Steam still reports `gameid`. Launch handoff + presence clear from earlier today kept. Deploy: `./recreate.sh`. Tests: `test/steam-now-playing-poller.test.js`.
