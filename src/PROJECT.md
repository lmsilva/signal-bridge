...
| `src/music-info.js` | Detect "play \<song\>" (`matchesMusicQuery`), "what song is playing" (`matchesNowPlayingQuery` — apostrophe-less ASR + spoken-answer fallback), and "next"/"skip" (`matchesMusicSkipQuery`); `fetchNowPlaying` / `fetchNowPlayingHousehold` / `resolveMusicQueryNowPlaying` (idle Echo → scan household + parse spoken "X by Y"); `fetchNowPlayingAfterSkip`; `isMusicPlayerContent` gates out flash briefing/news/Audible; `emptyNowPlaying` only when nothing is playing anywhere |
...
| `steam.now-playing` | Auto: persistent game card when the linked Steam account is in-game on **any PC** (`STEAM_REQUIRE_PRESENCE=0`, default — Steam’s API cannot name the machine). Detection: `GetPlayerSummaries.gameid`, else local presence, else **OwnedGames `rtime_last_played`** within `STEAM_INFER_FROM_RECENT_SEC` (default 300) when persona is online; recent-led sessions end after `STEAM_RECENT_PLAY_STAGNANT_SEC` (default 120) without playtime/rtime growth, and a cleared `gameid` is treated as authoritative quit (won’t resurrect from OwnedGames until relaunch). Artwork: `data/steam-artwork-cache/` + public `/steam-artwork/<appId>/…`; enrich uses cached store details/images first and warms in the background. Admin Settings → **Clear Steam artwork cache**. Optional host gate: `STEAM_REQUIRE_PRESENCE=1` + `STEAM_ALLOWED_HOSTS` + display.announce/`steamAppId`. Manual admin test skips host gate, dismissible, OwnedGames last-played fallback. API key: `.env` `STEAM_API_KEY` only. Interrupted by other display UDP → suppressed, then **auto-restored** after `STEAM_RESTORE_AFTER_INTERRUPT_SEC` (default 75) if still in-game. Close via `steam.now-playing.close` |
