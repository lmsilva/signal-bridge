...
**Last updated:** 2026-07-26 (Steam auto = gameid/presence only)
...
| `steam.now-playing` | Auto: persistent game card only when Steam’s profile reports **`gameid`** (account in-game) or a fresh **local presence** `steamAppId` arrives — never from OwnedGames last-played (quit stamps that timestamp and caused false “still playing” cards). `STEAM_REQUIRE_PRESENCE=0` (default) shows for any PC once `gameid` is set. Optional host gate: `STEAM_REQUIRE_PRESENCE=1` + `STEAM_ALLOWED_HOSTS` + display.announce/`steamAppId`. Manual Auth preview may use OwnedGames last-played (dismissible). API key: `.env` `STEAM_API_KEY` only. Interrupted by other display UDP → suppressed, then **auto-restored** after `STEAM_RESTORE_AFTER_INTERRUPT_SEC` (default 75) if still in-game. Close via `steam.now-playing.close` |
