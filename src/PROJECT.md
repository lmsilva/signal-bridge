...
**Last updated:** 2026-07-26 (Steam restore after interrupt)
...
| `steam.now-playing` | Auto: persistent game card when the linked Steam account is in-game on **any PC** (`STEAM_REQUIRE_PRESENCE=0`, default — Steam’s API cannot name the machine). Optional host gate: `STEAM_REQUIRE_PRESENCE=1` + `STEAM_ALLOWED_HOSTS` + display.announce/`steamAppId`. Manual admin test skips host gate, dismissible, last-played fallback. API key: `.env` `STEAM_API_KEY` only. Interrupted by other display UDP → suppressed, then **auto-restored** after `STEAM_RESTORE_AFTER_INTERRUPT_SEC` (default 75) if still in-game. Close via `steam.now-playing.close` |
