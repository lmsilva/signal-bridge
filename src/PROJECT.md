...
| `steam.now-playing` | Auto: persistent game card from (1) Steam profile **`gameid`**, else (2) fresh local presence `steamAppId`, else (3) OwnedGames activity that **advanced past an idle baseline** and is within `STEAM_INFER_FROM_RECENT_SEC` (default 180) — baseline is seeded on boot and updated on every session end so quit-time `rtime` cannot reopen the overlay. When `gameid` is absent, open sessions always run the OwnedGames stagnant check (`STEAM_RECENT_PLAY_STAGNANT_SEC`, default 150) even if local presence still claims an app (stuck `RunningAppID` must not block close); only Steam `gameid` refreshes the activity clock. Announce without `steamAppId` clears that host’s presence. `STEAM_REQUIRE_PRESENCE=0` (default) shows for any PC. Manual Auth preview may use OwnedGames last-played (dismissible). Close via `steam.now-playing.close` |
