...
| `src/slideshow-settings.js` | Persists the Shared Photo Slideshow's playback order (`recent` \| `oldest` \| `random`, default `recent`) to `data/slideshow-settings.json`, set from the web page's Settings tab |
...
| `POST /api/slideshow/settings` `{order}` | Persists playback order (`recent`\|`oldest`\|`random`) to `data/slideshow-settings.json` (Settings tab) |
