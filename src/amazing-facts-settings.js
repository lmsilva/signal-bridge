/**
 * Amazing Facts — house edits on top of the shipped corpus.
 *
 * Facts live in `amazing-facts-facts.json`. This file remembers hidden shipped
 * ids, text overrides, custom additions, optional category filters, and recent
...
function createAmazingFactsSettings(config = {}, log = console) {
  const settingsPath = config.amazingFactsSettingsPath
    || path.resolve(config.ROOT || path.resolve(__dirname, '..'), 'data', 'amazing-facts-settings.json');
  let current = {
    ...FALLBACK,
    overrides: {},
    custom: [],
  ...
  function load() {
    try {
      ...
      log?.warn?.('Could not read Amazing Facts settings', error?.message || error);
      current = sanitiseSettings({}, FALLBACK);
    }
    return current;
  }
  ...
  function save() {
    try {
      ...
      log?.warn?.('Could not save Amazing Facts settings', error?.message || error);
    }
  }
