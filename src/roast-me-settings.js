/**
 * Roast Me! — house edits on top of the shipped roasts.
 *
 * The roasts themselves live in `roast-me-roasts.json`. This file only
...
function createRoastMeSettings(config = {}, log = console) {
  const settingsPath = config.roastMeSettingsPath
    || path.resolve(config.ROOT || path.resolve(__dirname, '..'), 'data', 'roast-me-settings.json');
  ...
  function load() {
    try {
      ...
      log?.warn?.('Could not read Roast Me settings', error?.message || error);
      current = sanitiseSettings({}, FALLBACK);
    }
    return current;
  }
  ...
  function save() {
    try {
      ...
      log?.warn?.('Could not save Roast Me settings', error?.message || error);
    }
  }
