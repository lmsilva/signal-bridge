...
// "how much time left?" / "how much time is left on my timer" — Alexa often
// omits "is" and the word "timer"; treat those as an explicit timer list ask.
const SHOW_TIMERS_RE = /\b(?:show|list)\s+(?:me\s+)?(?:all\s+|my\s+)*timers?\b|\bwhat (?:are my timers|timers do i have)\b|\bhow much time(?:\s+do\s+i\s+have|\s+is)?\s+left\b|\bhow(?:'s|\s+is|\s+are)\s+(?:my\s+|the\s+)?timers?\b|\btime\s+left\s+on(?:\s+(?:my|the))?\s+timers?\b/i;
