/**
 * Daily Bucket Fillers — house edits on top of the shipped corpus.
 */
...
function createDailyBucketFillersSettings(config = {}, log = console) {
  const settingsPath = config.dailyBucketFillersSettingsPath
    || path.resolve(config.ROOT || path.resolve(__dirname, '..'), 'data', 'daily-bucket-fillers-settings.json');
  ...
  function load() {
    try {
      ...
      log?.warn?.('Could not read Daily Bucket Fillers settings', error?.message || error);
      current = sanitiseSettings({}, FALLBACK);
    }
    return current;
  }
  ...
  function save() {
    try {
      ...
      log?.warn?.('Could not save Daily Bucket Fillers settings', error?.message || error);
    }
  }
...
module.exports = {
  ...
  cleanText,
  sanitiseSettings,
  createDailyBucketFillersSettings,
};
