...
function isStaticAssetUrl(url) {
  ...
  return /\.(ico|js|mjs|ttf|otf|eot|woff2?|svg|png|jpe?g|gif|webp|appcache|map|bin)$/i.test(bare);
}

function isBinaryResponse(proxyRes) {
...
function injectAuthUiFix(html) {
  if (!html || typeof html !== 'string') return html;
  if (!/<html[\s>]/i.test(html)) return html;

  const fix = '<style id="alexa-auth-ui-fix">html,body,body *{font-family:system-ui,-apple-system,"Segoe UI",Roboto,Arial,sans-serif!important;font-stretch:normal!important;letter-spacing:normal!important;line-height:normal!important;transform:none!important}</style>';
  if (html.includes('</head>')) {
    return html.replace('</head>', `${fix}</head>`);
  }
  if (html.includes('<head>')) {
    return html.replace('<head>', `<head>${fix}`);
  }
  return `${fix}${html}`;
}
...
function customStringify(v, func, intent) {
  const cache = new Map();
  return JSON.stringify(v, function stringify(key, value) {
...
function initAmazonProxy(_options, callbackCookie, callbackListening) {
  ...
  if (!_options.formerRegistrationData || !_options.formerRegistrationData.deviceId || !formerDataStoreValid) {
    ...
    const bufHex = crypto.randomFillSync(buf).toString('hex').toUpperCase();
    deviceId = Buffer.from(bufHex).toString('hex');
  ...
  function router(req) {
    ...
    if (req.headers.host === `${_options.proxyOwnIp}:${_options.proxyPort}`) {
      ...
      if (url.startsWith(`/alexa.${_options.baseAmazonPage}/`)) {
      ...
      if (req.headers.referer) {
        ...
        if (req.headers.referer.startsWith(`http://${_options.proxyOwnIp}:${_options.proxyPort}/alexa.${_options.baseAmazonPage}/`)) {
          return `https://alexa.${_options.baseAmazonPage}`;
        }
      }
      if (url === '/') {
  ...
  function replaceHosts(data) {
    ...
    const protoAmazonRegex = new RegExp(`//www.${_options.baseAmazonPage}/`.replace(/\./g, '\\.'), 'g');
    const protoAlexaRegex = new RegExp(`//alexa.${_options.baseAmazonPage}/`.replace(/\./g, '\\.'), 'g');
    data = data.replace(/&#x2F;/g, '/');
    ...
    data = data.replace(protoAmazonRegex, `http://${_options.proxyOwnIp}:${_options.proxyPort}/www.${_options.baseAmazonPage}/`);
    data = data.replace(protoAlexaRegex, `http://${_options.proxyOwnIp}:${_options.proxyPort}/alexa.${_options.baseAmazonPage}/`);
    return data;
  }
  ...
  function onProxyRes(proxyRes, req, res) {
    if (isBinaryResponse(proxyRes)) return;
    ...
    modifyResponse(res, (proxyRes && proxyRes.headers ? proxyRes.headers['content-encoding'] || '' : ''), function rewriteBody(body) {
      if (!body) return body;
      body = replaceHosts(body);
      body = injectAuthUiFix(body);
      return body;
    });
  }
  ...
  app.use(myProxy);
  app.get('/cookie-success', function cookieSuccess(req, res) {
  ...
  const server = app.listen(_options.proxyPort, _options.proxyListenBind, function listening() {
