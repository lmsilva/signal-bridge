/**
 * Conversation Starters — house edits on top of the shipped corpus.
 *
 * Prompts live in `conversation-starters-prompts.json`. This file only
...
function createConversationStartersSettings(config = {}, log = console) {
  const settingsPath = config.conversationStartersSettingsPath
    || path.resolve(config.ROOT || path.resolve(__dirname, '..'), 'data', 'conversation-starters-settings.json');
  ...
  function load() {
    try {
      ...
      log?.warn?.('Could not read Conversation Starters settings', error?.message || error);
      current = sanitiseSettings({}, FALLBACK);
    }
    return current;
  }
  ...
  function save() {
    try {
      ...
      log?.warn?.('Could not save Conversation Starters settings', error?.message || error);
    }
  }
...
module.exports = {
  ...
  cleanText,
  sanitiseSettings,
  createConversationStartersSettings,
};
