/**
 * US State Facts — region filter and recent rotation.
 */
...
function sanitiseSettings(raw = {}, base = FALLBACK) {
  ...
  const regionSource = Array.isArray(incoming.regions) ? incoming.regions : base.regions;
  return {
    recentIds: uniqueIds(recentSource).slice(-RECENT_CAP),
    regions: uniqueIds(regionSource),
  };
}

function createUsStateFactsSettings(config = {}, log = console) {
  const settingsPath = config.usStateFactsSettingsPath
    || path.resolve(config.ROOT || path.resolve(__dirname, '..'), 'data', 'us-state-facts-settings.json');
  ...
  function load() {
    try {
      ...
      log?.warn?.('Could not read US State Facts settings', error?.message || error);
      current = sanitiseSettings({}, FALLBACK);
    }
    return current;
  }
  ...
  function save() {
    try {
      ...
      log?.warn?.('Could not save US State Facts settings', error?.message || error);
    }
  }
