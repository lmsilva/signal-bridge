/**
 * Misheard Lyrics — house edits on top of the shipped corpus.
 *
 * The lyrics themselves live in `misheard-lyrics-lyrics.json`. This file
 * only remembers hidden shipped ids, text/artist overrides, custom
 * additions, and the recent-id window so a scheduled rotation does not
 * repeat itself.
 */
...
function cleanArtist(value) {
  return String(value || '').replace(/\s+/g, ' ').trim().slice(0, ARTIST_MAX);
}
...
function sanitiseCustom(list) {
  ...
  for (const row of Array.isArray(list) ? list : []) {
    ...
    seen.add(id);
    out.push({ id, text, artist: cleanArtist(row?.artist) });
  }
  return out;
}
...
function sanitiseOverrides(value) {
  ...
  for (const [id, raw] of Object.entries(value)) {
    ...
    if (raw.artist != null) {
      patch.artist = cleanArtist(raw.artist);
    }
...
function createMisheardLyricsSettings(config = {}, log = console) {
  const settingsPath = config.misheardLyricsSettingsPath
    || path.resolve(config.ROOT || path.resolve(__dirname, '..'), 'data', 'misheard-lyrics-settings.json');
  ...
  function load() {
    try {
      ...
      log?.warn?.('Could not read Misheard Lyrics settings', error?.message || error);
      current = sanitiseSettings({}, FALLBACK);
    }
    return current;
  }
  ...
  function save() {
    try {
      ...
      log?.warn?.('Could not save Misheard Lyrics settings', error?.message || error);
    }
  }
