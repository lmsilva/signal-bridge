/**
 * Warm Fuzzies — house edits on top of the shipped corpus.
 */
...
function createWarmFuzziesSettings(config = {}, log = console) {
  const settingsPath = config.warmFuzziesSettingsPath
    || path.resolve(config.ROOT || path.resolve(__dirname, '..'), 'data', 'warm-fuzzies-settings.json');
  ...
  function load() {
    try {
      ...
      log?.warn?.('Could not read Warm Fuzzies settings', error?.message || error);
      current = sanitiseSettings({}, FALLBACK);
    }
    return current;
  }
  ...
  function save() {
    try {
      ...
      log?.warn?.('Could not save Warm Fuzzies settings', error?.message || error);
    }
  }
