/**
 * Plex token — env or encrypted admin save.
 * Precedence: PLEX_TOKEN → data/plex-credentials.json.
 */
...
function defaultCredentialsPath(root) {
  return path.resolve(root || path.resolve(__dirname, '..'), 'data', 'plex-credentials.json');
}
...
function loadPlexToken(credentialsPath, { secretBox = null } = {}) {
...
function savePlexToken(credentialsPath, token, { secretBox = null } = {}) {
...
function clearPlexToken(credentialsPath) {
  try {
...
function resolvePlexToken({
  env = process.env,
  credentialsPath,
} = {}) {
  const envKey = String(env.PLEX_TOKEN || '').trim();
  if (envKey) {
    return { token: envKey, tokenSource: 'env' };
  }
  const sessionKey = credentialsPath ? loadPlexToken(credentialsPath) : '';
  if (sessionKey) {
...
function credentialsStatus(credentialsPath, { env = process.env } = {}) {
  const resolved = resolvePlexToken({ env, credentialsPath });
  const raw = readCredentialsFile(credentialsPath);
  const stored = raw?.token;
  const keyUnreadable = Boolean(
    stored && !resolved.token && !String(env.PLEX_TOKEN || '').trim(),
  );
  ...
  return {
    ...
    envBlocksOverwrite: Boolean(String(env.PLEX_TOKEN || '').trim()),
  };
}
