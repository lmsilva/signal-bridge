...
(() => {
  ...
  // Search is a single-line textarea so Chrome's password manager does not
  // treat it as the household username after login. Enter must not wrap.
  function bindFilterSearch(input, { allowedValue = '', onClear } = {}) {
    if (!input) return;
    let typed = false;
    ...
    const wipeAutofill = () => {
      ...
      const allowed = String(allowedValue || '');
      if (current === allowed) return;
      input.value = allowed;
      onClear?.();
    };
    input.addEventListener('input', wipeAutofill);
    [0, 50, 200, 600, 1200].forEach((ms) => setTimeout(wipeAutofill, ms));
  }

  bindFilterSearch($('push-search'), { onClear: () => applyPushFilter() });
  bindFilterSearch($('settings-search'), {
    allowedValue: uiStorageGet(SETTINGS_SEARCH_KEY, ''),
    onClear: () => applySettingsFilter(),
  });
  ...
  // Tile artwork lives in /push-icons.js so the household Push tab draws the
  // same glyphs. The bridge still owns *what* can be pushed.
  const pushIconSvg = window.pushIconSvg;
  ...
  // The theme grids live in /red-letter-presets.js so the household event
  // editor paints the same starting points this designer does.
  const RL_PRESET_LIB = window.RED_LETTER_PRESETS;
