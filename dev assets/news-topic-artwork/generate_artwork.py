...
# --------------------------------------------------------------------------
CATEGORIES = [
    # guardian sectionId,  label,                 pattern,       background, tone
    ("environment",        "Environment",         "growth",      "#00582A", 0.95),
    ("science",            "Science",             "lissajous",   "#005362", 0.92),
    ("society",            "Society",             "handshake",   "#5B005C", 0.70),
    ("health",             "Health",              "pulse",       "#8C0030", 0.88),
    ("world",              "World",               "globe",       "#003F71", 0.80),
    ("global-development", "Global Development",  "ascend",      "#614700", 0.95),
    ("technology",         "Technology",          "circuit",     "#004044", 0.78),
    ("culture",            "Culture",             "ribbons",     "#9C0018", 0.95),
    ("education",          "Education",           "textblocks",  "#623000", 0.80),
    ("wildlife",           "Wildlife",            "cells",       "#003A01", 0.68),
    ("sport",              "Sport",               "courtarcs",   "#2A4A00", 0.85),
    ("business",           "Economy",             "network",     "#002966", 0.62),
    ("general",            "Good News",           "sunburst",    "#7A2396", 0.90),
]
...
def p_pulse(d, W, H, rng):
    """ECG / vitals trace — health."""
    for lane in range(6):
        cy = H * (0.12 + lane * 0.155)
        x = -W * 0.05
        amp = H * 0.052 * rng.uniform(0.7, 1.15)
        while x < W * 1.05:
            seg = W * rng.uniform(0.20, 0.30)
            pts = [(x, cy)]
            n = 90
            for i in range(n + 1):
                t = i / n
                ...
                if 0.30 < t < 0.34:
                    ...
                    py = cy - amp * 0.22 * math.sin((t - 0.52) / 0.10 * math.pi)
                else:
                    py = cy
                pts.append((px, py))
            d.line(pts, fill=205, width=3, joint="curve")
            x += seg


def p_growth(d, W, H, rng):
    """Sprouting stems and leaf pairs — environment."""
    for k in range(11):
        x = W * (0.02 + k * 0.098) + rng.uniform(-W * .02, W * .02)
        y = H * 1.02
        ang = -math.pi / 2 + rng.uniform(-0.12, 0.12)
        ln = min(W, H) * rng.uniform(0.55, 1.05)
        pts, steps = [(x, y)], 60
        curl = rng.uniform(-0.010, 0.010)
        for i in range(steps):
            ang += curl
            x += math.cos(ang) * ln / steps
            y += math.sin(ang) * ln / steps
            pts.append((x, y))
            if i % 9 == 4 and i > 6:
                for s in (-1, 1):
                    la = ang + s * rng.uniform(0.7, 1.1)
                    lr = ln * 0.16
                    ex, ey = x + math.cos(la) * lr, y + math.sin(la) * lr
                    mx = (x + ex) / 2 + math.cos(la + s * 1.57) * lr * 0.34
                    my = (y + ey) / 2 + math.sin(la + s * 1.57) * lr * 0.34
                    leaf = [(x, y)]
                    for j in range(15):
                        t = j / 14
                        leaf.append((
                            (1 - t) ** 2 * x + 2 * (1 - t) * t * mx + t * t * ex,
                            (1 - t) ** 2 * y + 2 * (1 - t) * t * my + t * t * ey))
                    for j in range(15):
                        t = 1 - j / 14
                        mx2 = (x + ex) / 2 - math.cos(la + s * 1.57) * lr * 0.34
                        my2 = (y + ey) / 2 - math.sin(la + s * 1.57) * lr * 0.34
                        leaf.append((
                            (1 - t) ** 2 * x + 2 * (1 - t) * t * mx2 + t * t * ex,
                            (1 - t) ** 2 * y + 2 * (1 - t) * t * my2 + t * t * ey))
                    d.line(leaf, fill=170, width=2, joint="curve")
        d.line(pts, fill=210, width=3, joint="curve")


def p_ascend(d, W, H, rng):
    """Rising columns and a trend line — development / progress."""
    n = 22
    bw = W / (n + 2)
    base = H * 0.94
    prev = None
    for i in range(n):
        t = i / (n - 1)
        h = H * (0.10 + 0.62 * t ** 1.15) * rng.uniform(0.86, 1.10)
        x = bw * (i + 1)
        d.rectangle([x, base - h, x + bw * 0.62, base], outline=130, width=2)
        top = (x + bw * 0.31, base - h)
        if prev:
            d.line([prev, top], fill=215, width=3)
        d.ellipse([top[0] - 4, top[1] - 4, top[0] + 4, top[1] + 4], fill=235)
        prev = top
    d.line([0, base, W, base], fill=85, width=3)


def p_handshake(d, W, H, rng):
    """Interlocking arcs — society / cooperation."""
    for k in range(11):
        cx = W * rng.uniform(0.10, 0.90)
        cy = H * rng.uniform(0.12, 0.88)
        r = min(W, H) * rng.uniform(0.16, 0.36)
        a0 = rng.uniform(0, 360)
        for i in range(3):
            rr = r * (1 - i * 0.22)
            d.arc([cx - rr, cy - rr, cx + rr, cy + rr], a0, a0 + 250,
                  fill=190 - i * 40, width=3)


def p_globe(d, W, H, rng):
    """Graticule sphere — world."""
    cx, cy = W * 0.5, H * 0.48
    R = min(W, H) * 0.40
    d.ellipse([cx - R, cy - R, cx + R, cy + R], outline=200, width=3)
    for i in range(1, 8):                      # parallels
        f = -1 + 2 * i / 8
        ry = R * math.sqrt(max(1 - f * f, 0))
        yy = cy + f * R
        d.arc([cx - R, yy - ry * 0.30, cx + R, yy + ry * 0.30], 0, 360,
              fill=120, width=2)
    for i in range(9):                          # meridians
        f = -1 + 2 * i / 8
        rx = R * abs(f)
        d.arc([cx - rx, cy - R, cx + rx, cy + R], 0, 360, fill=120, width=2)
...
def save_verified(img, path):
    ...
    if ext in (".jpg", ".jpeg"):
        save_kw = {"format": "JPEG", "quality": 90, "optimize": True}
    else:
        save_kw = {"format": "WEBP", "quality": 92, "method": 4}
...
def main():
    ...
    # Default: this folder. Optional 3rd arg = output root for shipping copies.
    root = os.path.abspath(sys.argv[3]) if len(sys.argv) > 3 else os.path.dirname(os.path.abspath(__file__))
    os.makedirs(root, exist_ok=True)
    mpath = os.path.join(root, "categories.json")
    manifest = json.load(open(mpath, encoding="utf-8"))["categories"] if os.path.exists(mpath) else []
    for idx, (cid, label, pattern, bg_hex, tone) in list(enumerate(CATEGORIES))[lo:hi]:
        ...
        for orient, (W, H) in (("portrait", (1080, 1920)), ("landscape", (1920, 1080))):
            ...
            # JPEG ships with the portable client (Pillow often lacks WebP).
            name = f"{cid}-{orient}.jpg"
        ...
        print(f"{idx+1:2d}/13 {cid:22s} bg {bg_hex}  accent {entry['accent']}  "
    ...
    print(f"\n{len(manifest)} categories, {len(manifest) * 2} images -> {root}")
